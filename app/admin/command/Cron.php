...
class Cron extends \think\console\Command
{
    ...
    {
        $invoiceids = \think\Db::name("invoices")->where("status", "Unpaid")->where("is_delete", 1)->where("create_time", "<", time() - 7200)->column("id");
        ...
        if ($cron_order_unpaid_time_high == 1 && 0 < $day) {
            $before_day_start_time = strtotime("-" . $day . " days", strtotime(date("Y-m-d")));
            $before_day_end_time = strtotime("+1 days -1 seconds", $before_day_start_time);
            $orders = \think\Db::name("orders")->alias("a")->leftJoin("invoices b", "a.invoiceid = b.id")->field("a.id,a.uid,b.id as invoiceid,b.create_time")->whereBetweenTime("a.create_time", $before_day_start_time, $before_day_end_time)->where("b.status", "Unpaid")->where("a.delete_time", 0)->where("b.delete_time", 0)->select()->toArray();
        ...
        foreach ($host as $v) {
            ...
            if ($v["bill_type"] == "last_30days") {
                ...
                for ($max = $i + 30; $i < $max; $i++) {
                    $target = date("Y-m-d", strtotime(date("Y-m-d", $row["regdate"]) . " +" . $i . " month"));
                    if ($target == $date) {
        ...
        foreach ($res as $k => $v) {
            if ($v["status"] == 200 && $v["http_code"] == 200 && !empty($v["data"]["meta"])) {
                ...
                if ($v["data"]["meta"]["traffic_type"] == 1) {
                    ...
                    if ($v["data"]["meta"]["traffic_type"] == 2) {
                        $usage = round($v["data"]["info"]["30_day"]["send"] / pow(1024, 3), 2);
                    } else {
                        if ($v["data"]["meta"]["traffic_type"] == 3) {
