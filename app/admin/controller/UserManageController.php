...
{
    private $saveImage = NULL;
    private $getImage = NULL;
    private $cardType = NULL;
    private $getAvatar = NULL;
    ...
    public function hostByUid()
    {
        ...
        if ($params["source"] == "ticket") {
            ...
            $hosts = \think\Db::name("host")->alias("h")->field("h.id as hid,p.name,h.domain,h.amount,h.firstpaymentamount,h.dedicatedip,h.billingcycle,h.regdate,h.nextduedate,h.domainstatus,p.type as host_type,p.type as ptype")->leftJoin("products p", "h.productid = p.id")->where("uid", $uid)->withAttr("host_type", function ($value) {
    ...
    public function profilePost()
    {
        if ($this->request->isPost()) {
            $params = $this->request->only(["client_id", "username", "password", "sex", "avatar", "profession", "signature", "companyname", "email", "country", "province", "city", "region", "address1", "postcode", "phone_code", "phonenumber", "currency", "defaultgateway", "notes", "qq", "sale_id", "groupid", "status", "language", "know_us", "custom", "marketing_emails_opt_in", "aff_id_uid"]);
